...
begin
  if sOpr = 'INE' then begin
    if not(frmMain.stpPermissao.FieldByName('Incluir endereços').AsBoolean) then begin
    ...
    if sOpr = 'ACE' then begin
      if not(frmMain.stpPermissao.FieldByName('Alterar endereços').AsBoolean) then begin
        ALERTMSG('Erro de Permissão','O seu usuário não possui permissão para acessar esse módulo do sistema.',false);
        PostMessage(frmIAEEndereco.Handle, WM_CLOSE, 0, 0)
      end;
      ...
      if not(frmMain.stpPermissao.FieldByName('Excluir endereços').AsBoolean) then begin
        ALERTMSG('Erro de Permissão','O seu usuário não possui permissão para acessar esse módulo do sistema.',false);
        PostMessage(frmIAEEndereco.Handle, WM_CLOSE, 0, 0)
      end;
