...
begin
  ...
  if stpLogin.RecordCount = 0 then begin
    ...
    if stpLogin.Fields.FieldByName('id_Codigo_Usu').AsInteger = 13 then begin
      bLoginReturn := False;
      ALERTMSG('FALHA DE AUTENTICAÇÃO','Houve um problema na autenticação do sistema e ele não pôde ser iniciado, favor entrar em contato com o administrador do sistema.',false);
      edtSenha.Text := '';
      edtLogin.SetFocus;
      exit;
    end;
